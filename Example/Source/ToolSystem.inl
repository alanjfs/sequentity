...
void TranslateSystem() {
    ...
    auto update = [](const auto& data, const auto& intent) {
        ...
        if (!Registry.has<MoveIntent>(data.target)) {
            Registry.assign<MoveIntent>(data.target, position.delta.x(), position.delta.y());
        }
...
void RotateSystem() {
    ...
    auto update = [](const auto& data, const auto& intent) {
        ...
        if (!Registry.has<RotateIntent>(data.target)) {
            Registry.assign<RotateIntent>(data.target, position.delta.x());
...
void ScaleSystem() {
    ...
    auto update = [](const auto& data, const auto& intent) {
        ...
        if (!Registry.has<ScaleIntent>(data.target)) {
            Registry.assign<ScaleIntent>(data.target, position.delta.x());
